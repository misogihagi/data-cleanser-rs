...
async fn get_term(url: &str) -> reqwest::Result<Term> {
    ...
    loop {
        ...
        match res {
            ...
            Err(err) => {
                if err.source().unwrap().source().unwrap().to_string()
    ...
    let body_element = fragment.select(&body_selector).next().unwrap();
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let idxs: Vec<String> = get_indexes()
        .await?
        .iter()
        .map(|x| host.clone() + x)
        .collect();

    let term_urls: Vec<String> = join_all(idxs.iter().map(|x| get_terms(x)))
        .await
        .into_iter()
        .flat_map(|x| x.unwrap())
        .collect();

    let t: Terms = join_all(term_urls.iter().map(|x| get_term(x)))
        .await
        .into_iter()
        .map(|x| x.unwrap())
        .collect();
